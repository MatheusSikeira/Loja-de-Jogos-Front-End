...
router.post('/inserirJogo', async (req, res) => {
    try {
        const { nome, desenvolvedor, plataforma, preco, imagem_url, usuario_id, cod_categoria } = req.body;
        
        if (!nome || !desenvolvedor || !plataforma || !preco || !usuario_id || !cod_categoria) {
...
// Obter jogo por ID
router.get('/listagemJogo/:id_jogo', async (req, res) => {
    try {
        const jogo = await modelJogo.findByPk(req.params.id_jogo);
...
// Excluir jogo
router.delete('/excluirJogo/:id_jogo', async (req, res) => {
    try {
        const resultado = await modelJogo.destroy({
            where: { id_jogo: req.params.id_jogo }
        });
...
router.put('/alterarJogo', async (req, res) => {
    try {
        const { id_jogo, ...camposAtualizar } = req.body;
